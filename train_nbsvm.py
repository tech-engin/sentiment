...
def tokenize(sentence, grams):
    words = clear_review_to_words(sentence)
    tokens = []
    for gram in grams:
...
def generate_Xy_nbsvm(labeled_data,test_data,model_path=(None,None)):
    X_train_nbsvm, _, X_test_nbsvm, _ = load_svmlight_files(model_path)
    y_labeled=labeled_data.sentiment.values
    id_test = test_data.id.values
    return X_train_nbsvm,X_test_nbsvm,y_labeled.tolist(),id_test